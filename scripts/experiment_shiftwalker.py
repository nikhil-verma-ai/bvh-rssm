#!/usr/bin/env python3
"""
ShiftWalker experiment — demonstrates C-index on a MuJoCo friction-shift env.

ShiftWalker (FNSB Env 2): Walker2d-v4 with Poisson-triggered friction shifts
between low/medium/high ground friction values.  Shifts are abrupt; oracle_tau
is the steps until the next friction-change event.  Because shifts are Poisson
(memoryless), this is harder than SensorDrift but the latent should still pick
up gait/stability signals that correlate with impending traction loss.

Contrast with ShiftPendulum (also Poisson) — adds a second MuJoCo control env
to the BVH-RSSM benchmark suite, confirming generality across locomotion tasks.

Usage
-----
    python scripts/experiment_shiftwalker.py                   # full run
    python scripts/experiment_shiftwalker.py --steps 300       # smoke test
    python scripts/experiment_shiftwalker.py --skip-p1 checkpoints/sw_p1.pt

Outputs
-------
    checkpoints/sw_p1.pt              — Phase 1 checkpoint (save with --save-p1)
    results/shiftwalker_report.json   — full metrics
"""
from __future__ import annotations

# ── MuJoCo guard ─────────────────────────────────────────────────────────────
# Must run before any gymnasium/mujoco import so we surface a clear message
# instead of a cryptic ImportError deep in the call stack.
try:
    import mujoco  # noqa: F401
except ImportError:
    print(
        "[ShiftWalker] MuJoCo is not installed.\n"
        "  Install with:  pip install mujoco\n"
        "  and ensure a valid MuJoCo license / free licence key is set up.\n"
        "Exiting gracefully — no crash."
    )
    import sys
    sys.exit(0)
...
# Survival horizon: cap at K=16 steps (same as SensorDrift/ShiftPendulum).
# Poisson shifts mean τ* ∈ [0, ∞) but we treat any τ* ≥ K as right-censored.
K = 16

# Walker2d-v4 observation/action space (confirmed from MuJoCo XML):
#   obs: 17-dim  (qpos[1:] = 8 joints, qvel = 9)
#   act:  6-dim  (6 actuators for hip/knee/ankle × 2 legs)
OBS_DIM    = 17
ACTION_DIM = 6
...
def _parse():
    p = argparse.ArgumentParser(
        description="BVH-RSSM ShiftWalker experiment (MuJoCo friction shifts)"
    )
    ...
    p.add_argument("--out", type=str, default="results/shiftwalker_report.json")
    return p.parse_args()
...
class SWStepper:
    """Single-environment stepper for ShiftWalker."""

    def __init__(self, seed: int, device: torch.device):
        self._env    = ShiftWalker(shift_rate=5.0, shift_type="abrupt", seed=seed)
        self._device = device
...
def compute_eval_metrics(model, device, n_episodes=50, max_steps=500):
    rssm, enc            = model["rssm"], model["encoder"]
    ...
    for ep in range(n_episodes):
        env   = ShiftWalker(shift_rate=5.0, shift_type="abrupt", seed=ep * 100)
    ...
    # Negating: higher score = lower mse = model more confident = higher τ.
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  BVH-RSSM ShiftWalker Experiment — device={device_str}")
    print(f"  env=Walker2d-v4  friction_shifts=abrupt/Poisson  K={K}")
    if skip_p1:
    ...
    stepper = SWStepper(seed=args.seed, device=device)
    ...
    metrics = compute_eval_metrics(model, device)
    ...
    pf(metrics["c_index"] > 0.65,
       f"C-index > 0.65: {metrics['c_index']:.4f}  ← THE BREAKTHROUGH")
    print(f"{'='*70}")
